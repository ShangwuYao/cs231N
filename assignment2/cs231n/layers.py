...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        sample_var = np.std(x, axis = 0, keepdims=True)**2    # [1,D]
...
def batchnorm_backward(dout, cache):
    ...
    sample_std_inv = 1.0 / np.sqrt(sample_var + eps) # (D,)
    ...
    dsample_mean = -1.0 * np.sum(dx_normalized * sample_std_inv, axis = 0, keepdims=True) - \
                        2.0 * dsample_var * np.mean(x_mu, axis=0, keepdims=True) # (1,D)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        temp_x = x
        random = np.random.rand(*temp_x.shape)
        mask = random * (random > p)
        out = temp_x * mask 
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def conv_backward_naive(dout, cache):
    ...
    for f in range(F):
        for c in range(C):
            for j in range(HH):
                for k in range(WW):
                    x_subpad = x_pad[:, c, j:j+H_new*s:s, k:k+W_new*s:s]
                    dw[f, c, j, k] = np.sum(dout[:, f, :, :] * x_subpad)
                ...
                for f in range(F):
                    for k in range(H_new):
                        for l in range(W_new):
                            ...
                            if (i + pad - k * s) < HH and (i + pad - k * s) >= 0:
                                mask1[:, i + pad - k * s, :] = 1.0
                            if (j + pad - l * s) < WW and (j + pad - l * s) >= 0:
                                mask2[:, :, j + pad - l * s] = 1.0
                            w_masked = np.sum(
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_new = int((H - pool_height)/s + 1)
    W_new = int((W - pool_width)/s + 1)
        ...
        for c in range(C):
            for i in range(H_new):
                for j in range(W_new):
                    pool = x[n, c, i*s:i*s+pool_height, j*s:j*s+pool_width] 
        ...
        for c in range(C):
            for h in range(H_new):
                for w in range(W_new):
                    pool = x[n, c, h*s:h*s+pool_height, w*s:w*s+pool_width] 
                    a, b = np.unravel_index(pool.argmax(), pool.shape)
                    a += h*s
                    b += w*s
                    dx[n, c, a, b] += dout[n, c, h, w]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
